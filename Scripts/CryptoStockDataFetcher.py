...
def get_stock_data(ticker, date):
    ...
    #NewsData.io URL
    url = f"https://api.polygon.io/v1/open-close/crypto/{ticker}/USD/{date}?adjusted=true&apiKey={STOCKDATA_API_KEY}"
    params = {"language" : "en"}
...
def write_to_csv(data, file="StockData.csv", category = "None"):
    ...
    with open(filepath, filemode) as data_csv:
        ...
        if(filemode == "w+"):
            csv_writer.writerow(["CATEGORY", "TICKER", "DATE", "OPEN", "CLOSE"])
        
        if(data['symbol'] != None):
            csv_writer.writerow([category, data["symbol"], data["day"][:10], data["open"], data["close"]])
        else:
            print("NOT OK")
        data_csv.close()
...
def main(file):
    ...
    next_date = datetime.datetime(2021, 12, 19)
